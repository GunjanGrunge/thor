...
REMOTE_PATH="${REMOTE_PATH:-/home/ubuntu/scripts/remote_thor_verify_runtime.sh}"
LOCAL_SCRIPT="/mnt/c/Users/Bot/Desktop/Thor/scripts/remote_thor_verify_runtime.sh"
KEY_DEST="${KEY_DEST:-$HOME/.ssh/thor-training-20260416-184056.pem}"

mkdir -p /tmp/thor-upload
cp "$LOCAL_SCRIPT" /tmp/thor-upload/remote_thor_verify_runtime.sh
...
scp -o StrictHostKeyChecking=accept-new -i "$KEY_DEST" \
  /tmp/thor-upload/remote_thor_verify_runtime.sh "$REMOTE_USER@$HOST:$REMOTE_PATH"
