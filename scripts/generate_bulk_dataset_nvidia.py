...
def get_query_embedding(query: str) -> np.ndarray:
    ...
    return vec_np / max(norm, 1e-9)

def retrieve(query: str, top_k: int) -> list[dict]:
    base_dir = EMBED_DIR / sanitize_model_name(EMBED_MODEL)
    ...
    if not metadata_path.exists() or not embeddings_path.exists():
        raise FileNotFoundError(f"Embeddings not found at {base_dir}")
        
    metadata = list(jsonlines.open(metadata_path, mode="r"))
...
def build_prompt(seed: dict, evidence: list[dict]) -> str:
    evidence_block = []
...
def generate_row(prompt: str) -> dict:
    res = requests.post(
    ...
    try:
        ...
        print("Failed to decode JSON from Llama:")
        print(content)
        raise

def main():
    seed_config = CONFIG_DIR / "grounded_generation_seeds_v4.json"
    seeds = json.loads(seed_config.read_text(encoding="utf-8"))["seeds"]
    
    out_file = SFT_DIR / "nvidia_grounded_examples.jsonl"
    out_file.parent.mkdir(parents=True, exist_ok=True)
    
    outputs = []
    
    print(f"Beginning NVIDIA generation block for {len(seeds)} seeds...")
    for idx, seed in enumerate(seeds, start=1):
        print(f"[{idx}/{len(seeds)}] Generating generic scenario: {seed['id']}")
        try:
            evidence = retrieve(seed["user_query"], top_k=6)
            prompt = build_prompt(seed, evidence)
            
            # NVIDIA chat completion allows ~60 rpm.
            generated = generate_row(prompt)
            ...
            print(f"  FAILED on {seed['id']}: {e}")

    print(f"\nSuccessfully generated {len(outputs)} SFT rows out of {len(seeds)}!")
